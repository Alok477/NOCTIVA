...
const cacheKey = (uid) => `noctiva_cw_cache_items_${uid}`;
...
export default function ContinueWatchingRow({ onSelect }) {
  ...
  useEffect(() => {
    return onAuthStateChanged(auth, (u) => {
      ...
      try {
        const cached = JSON.parse(localStorage.getItem(cacheKey(u.uid)) || '[]');
        setItems(Array.isArray(cached) ? cached : []);
      } catch (error) {
        console.warn('Unable to restore continue-watching cache:', error);
        setItems([]);
      }
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snapshot) => {
      ...
      setItems(data);
      try {
        localStorage.setItem(cacheKey(user.uid), JSON.stringify(data));
      } catch (error) {
        console.warn('Unable to cache continue-watching items:', error);
      }
    }, (error) => {
      console.error('Failed to sync continue-watching items:', error);
    });
    return unsub;
  }, [user]);
