...
import { FaSignOutAlt, FaUserCircle, FaMagic } from 'react-icons/fa';
...
function Sidebar({ activePage, onNavigate, selectedGenreId, onGenreSelect, onOpenAuthModal, onLogout, user }) {

  const ACTIVE_MAP = { search: 'search', movies: 'movies', series: 'series', watchlist: 'watchlist', 'match-mood': 'match-mood', home: 'home' };
  ...
  return (
    ...
    ">
      ...
      <button onClick={() => onNavigate('home')} className="relative flex items-center gap-4 px-[18px] pt-8 pb-8 shrink-0 text-left hover:opacity-90 transition-opacity">
        <div className="flex items-center justify-center w-[48px] h-[48px] rounded-2xl bg-black shadow-lg shadow-red-900/40 ring-1 ring-white/10 shrink-0 overflow-hidden">
        ...
        <div className="flex flex-col leading-tight whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-200 delay-100">
          <span className="text-white font-black text-[20px] tracking-tight">NOCTIVA</span>
          <span className="text-red-400/70 text-[10px] font-semibold tracking-[0.22em] uppercase mt-0.5">Discover &amp; Track</span>
        </div>
      </button>
