...
import { fetchMovieDetails, fetchRelatedMovies, fetchMovieWatchProviders } from "../Fetcher";
...
const MovieDetails = ({ movieId: movieIdProp }) => {
  ...
  const load = useCallback(async () => {
    ...
    setRetrying(true);
    try {
      const [data, relatedData, detailLogoPath, providersData] = await Promise.all([
        fetchMovieDetails(movieId),
        fetchRelatedMovies(movieId),
        fetchLogoPath('movie', movieId),
      ...
      setWatchProviders(providersData ?? null);
    } catch {
      if (requestId !== loadRequestRef.current) return;
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(collection(db, 'reviews'), (snapshot) => {
      const nextReviews = snapshot.docs
        ...
        .filter((item) => item.mediaId === String(movie.id) && item.mediaType === 'movie')
        .sort((a, b) => {
  ...
  const lastUpdated = countryProviders?.last_updated || watchProviders?.last_updated || watchProviders?.results?.IN?.last_updated;
          ...
          <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
            ...
            <form onSubmit={handleReviewSubmit} className="rounded-[2rem] border border-white/10 bg-white/[0.02] p-5 sm:p-6">
              ...
              <p className="mt-1 text-sm text-gray-400">
                {user ? 'Share your experience with this movie.' : 'Sign in to add your review.'}
              </p>
              ...
              <div className="mt-5">
                ...
                <textarea
                  ...
                  rows={6}
                  maxLength={500}
                  placeholder={user ? 'What did you like or dislike about this movie?' : 'Please sign in to leave a review.'}
