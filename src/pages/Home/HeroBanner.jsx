      ...
      try {
        ...
        if (!cancelled) {
          ...
          const itemsWithLogos = await Promise.all(
            bannerItems.map(async (item) => {
              try {
                const images = await fetch(
                  `${BASE_URL}/${item.media_type}/${item.id}/images?api_key=${API_KEY}&include_image_language=en,null`
                ).then(response => response.json());
                const logo = (images.logos ?? []).find(image => image.iso_639_1 === 'en')
                  ?? (images.logos ?? [])[0];
                return { ...item, logo_path: logo?.file_path ?? null };
              } catch {
          ...
          if (!cancelled) setItems(itemsWithLogos);
        }
...
export default function HeroBanner() {
  ...
  if (loading) {
    return (
      <div className="hero-banner relative w-full h-[72vh] md:h-screen overflow-hidden bg-[#0a0c12]">
  ...
  return (
    <div className="hero-banner relative w-full h-[72vh] md:h-screen overflow-hidden bg-black select-none">
      ...
      <div className={`absolute inset-0 transition-opacity duration-500 ${fade ? 'opacity-100' : 'opacity-0'}`}>
        ...
        />
        {/* Cinematic overlays */}
        <div className="hero-overlay-main absolute inset-0 bg-gradient-to-r from-black via-black/70 to-transparent" />
        <div className="hero-overlay-bottom absolute inset-0 bg-gradient-to-t from-[#0a0c12] via-transparent to-black/30" />
        <div className="hero-overlay-left absolute inset-y-0 left-0 w-1/2 bg-gradient-to-r from-black/60 to-transparent" />
      </div>
      ...
      <div className={`relative z-10 h-full flex flex-col justify-end md:justify-center px-6 md:px-14 pb-20 md:pb-16 max-w-2xl transition-opacity duration-500 ${fade ? 'opacity-100' : 'opacity-0'}`}>
        ...
        {logoPath ? (
          <img
            src={`${LOGO}${logoPath}`}
            alt={title}
            className="hero-title-logo w-auto max-w-[min(78vw,24rem)] max-h-24 md:max-h-32 object-contain object-left mb-5 drop-shadow-[0_2px_20px_rgba(0,0,0,0.8)]"
          />
        ) : (
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mb-5">
          ...
          {genres.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {genres.map(g => (
                <span key={g} className="hero-genre-pill text-[11px] font-semibold text-gray-300 bg-white/[0.09] border border-white/[0.12] px-2.5 py-0.5 rounded-full">
                  {g}
                </span>
              ))}
            </div>
        ...
        {/* Overview */}
        {overview && (
          <p className="hero-description text-gray-300/85 text-sm leading-relaxed mb-7 max-w-lg hidden sm:block">
            {overview}
          </p>
        )}
